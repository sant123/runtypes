...
type MergedRecordReadonly<O extends { [_: string]: RuntypeBase }> =
  ...
  & {
    [K in FilterOptionalKeys<O>]?: Static<O[K]>;
  } extends infer P ? { readonly [K in keyof P]: P[K] }
  : never;
...
> = Part extends true
  ...
  : RO extends true ? MergedRecordReadonly<O>
  : MergedRecord<O>;
  ...
  extend<P extends { [_: string]: RuntypeBase }>(
    fields: {
      [K in keyof P]: K extends keyof O
        ? Static<P[K]> extends Static<O[K]> ? P[K]
        : RuntypeBase<Static<O[K]>>
        : P[K];
    },
  ): InternalRecord<
    {
      [K in keyof (O & P)]: K extends keyof P ? P[K]
...
export type Record<O extends { [_: string]: RuntypeBase }, RO extends boolean> =
  InternalRecord<
    O,
    false,
    RO
          ...
          if (fieldsHasKey) {
            ...
            if (xHasKey) {
              ...
              if (isOptional && value === undefined) {
                ...
                results[key as any] = innerValidate(runtype, value, visited);
              }
            } else {
              if (!isOptional) {
                results[key as any] = FAILURE.PROPERTY_MISSING(runtype.reflect);
...
export function Record<O extends { [_: string]: RuntypeBase }>(
  fields: O,
): Record<O, false> {
  return InternalRecord(fields, false, false);
}

export function Partial<O extends { [_: string]: RuntypeBase }>(
  fields: O,
): Partial<O, false> {
...
>(A: any): InternalRecord<O, Part, RO> {
  ...
  function omit<K extends keyof O>(
    ...
    existingKeys.forEach((key) => {
      if (!(keys as (string | symbol)[]).includes(key)) {
        result[key] = A.fields[key];
      }
    });
    return InternalRecord(result, A.isPartial, A.isReadonly);
