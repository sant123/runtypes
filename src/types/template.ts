...
import { type Literal, literal, type LiteralBase } from "./literal.ts";
...
export interface Template<
  ...
  B extends readonly RuntypeBase<LiteralBase>[],
> extends
  Runtype<A extends TemplateStringsArray ? string : TemplateLiteralType<A, B>> {
  tag: "template";
  strings: A;
  runtypes: B;
}
...
> = A extends readonly [infer carA, ...infer cdrA]
  ? cdrA extends readonly any[]
    ? carA extends RuntypeBase<LiteralBase> ? [prefix, ...ExtractStrings<cdrA>] // Omit `carA` if it's a `RuntypeBase<LiteralBase>`
    : carA extends LiteralBase ? [...ExtractStrings<cdrA, `${prefix}${carA}`>]
...
> = A extends readonly [infer carA, ...infer cdrA]
  ? cdrA extends readonly any[]
    ? carA extends RuntypeBase<LiteralBase> ? [carA, ...ExtractRuntypes<cdrA>]
    : carA extends LiteralBase ? [...ExtractRuntypes<cdrA>]
...
const parseArgs = (
  ...
  if (0 < args.length && Array.isArray(args[0])) {
    ...
    const convenient =
      args as readonly (LiteralBase | RuntypeBase<LiteralBase>)[];
...
const flattenInnerRuntypes = (
  ...
  for (let i = 0; i < runtypes.length;) {
    switch (runtypes[i].reflect.tag) {
      ...
      case "union": {
        const union = runtypes[i] as unknown as Union<
          readonly [RuntypeBase<unknown>, ...RuntypeBase<unknown>[]]
        >;
        if (union.alternatives.length === 1) {
          try {
            const literal = getInnerLiteral(
              union as unknown as RuntypeBase<unknown>,
            );
            runtypes.splice(i, 1);
            const string = String(literal.value);
            strings.splice(i, 2, strings[i] + string + strings[i + 1]);
...
const reviveValidate = (reflect: Reflect, visited: VisitedState) =>
  (
    ...
    if (Array.isArray(revivers)) {
      switch (reflect.tag) {
        case "union":
          for (const alternative of reflect.alternatives) {
            const validated = reviveValidate(alternative.reflect, visited)(
              value,
            );
        ...
        case "intersect":
          for (const intersectee of reflect.intersectees) {
            const validated = reviveValidate(intersectee.reflect, visited)(
              value,
            );
            if (!validated.success) return validated;
          }
      ...
      if (
        !validated.success && validated.code === "VALUE_INCORRECT" &&
        reflect.tag === "literal"
      ) {
...
const createRegExpForTemplate = (reflect: Reflect & { tag: "template" }) => {
  const pattern = reflect.strings.map(escapeRegExp).reduce(
    (pattern, string, i) => {
      const prefix = pattern + string;
      const runtype = reflect.runtypes[i];
...
>(
  ...
  : A extends [infer carA, ...infer cdrA]
    ? carA extends readonly [string, ...string[]]
      ? cdrA extends readonly RuntypeBase<LiteralBase>[] ? Template<carA, cdrA> // For convenient parameter style
      : never
    : never
  : never {
  ...
  const test = (value: string, visited: VisitedState): Result<string> => {
    ...
    if (matches) {
      ...
      for (let i = 0; i < runtypes.length; i++) {
        ...
        const validated = reviveValidate(runtype.reflect, visited)(
          value,
        ) as Result<string>;
        if (!validated.success) return validated;
      }
