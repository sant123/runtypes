...
type StringLiteralFor<K extends DictionaryKeyType> = K extends string ? "string"
...
export interface Dictionary<V extends RuntypeBase, K extends DictionaryKeyType>
  extends
    Runtype<
      V extends Optional<any> ? { [_ in K]?: Static<V> }
        : { [_ in K]: Static<V> }
    > {
  tag: "dictionary";
...
export interface StringDictionary<V extends RuntypeBase> extends
  Runtype<
    V extends Optional<any> ? { [_ in string]?: Static<V> }
...
export interface NumberDictionary<V extends RuntypeBase> extends
  Runtype<
    V extends Optional<any> ? { [_ in number]?: Static<V> }
...
>(
  value: V,
  key?: K,
): K extends DictionaryKeyRuntype ? Dictionary<V, Static<K>>
  : K extends "string" ? StringDictionary<V>
  : K extends "number" ? NumberDictionary<V>
  : never {
  const keyRuntype = key === undefined
    ? String
    : key === "string"
    ? String
    : key === "number"
  ...
  return create<any>((x, visited) => {
    ...
    const results = keys.reduce<
      { [key in string | number | symbol]: Result<unknown> }
    >(
      (results, key) => {
        ...
        const isNumberLikeKey = typeof key === "string" &&
          numberString.test(key);
        ...
        if (
          ...
          results[key as any] = FAILURE.KEY_INCORRECT(
            self,
            keyRuntype.reflect,
            keyInterop,
          );
