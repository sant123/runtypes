...
import { hasKey, SUCCESS } from "./util.ts";

const RuntypeSymbol: unique symbol = Symbol();

export const isRuntype = (x: any): x is RuntypeBase<unknown> =>
  hasKey(RuntypeSymbol, x);
...
export type VisitedState = {
  has: (candidate: Record<string, unknown>, type: RuntypeBase) => boolean;
};
function VisitedState(): VisitedState {
  const members: WeakMap<Record<string, unknown>, WeakMap<RuntypeBase, true>> =
    new WeakMap();

  const add = (candidate: Record<string, unknown>, type: RuntypeBase) => {
  ...
  const has = (candidate: Record<string, unknown>, type: RuntypeBase) => {
    const typeSet = members.get(candidate);
