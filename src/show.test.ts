...
const cases: [Reflect, string][] = [
  ...
  [
    Template("foo", Template`foo${Literal("bar")}baz`, "baz"),
    '"foofoobarbazbaz"',
  ],
  [
    Template("foo", Union(Literal("foo"), Literal("bar")), "baz"),
    '`foo${"foo" | "bar"}baz`',
  ],
  ...
  [
    Record({ x: Number, y: Union(Number, Undefined) }),
    "{ x: number; y: number | undefined; }",
  ],
  [
    Record({ x: Number }).And(Partial({ y: Number })),
    "{ x: number; } & { y?: number; }",
  ],
  [
  ...
  [
    Record({ x: String, y: Array(Boolean) }).asPartial(),
    "{ x?: string; y?: boolean[]; }",
  ],
